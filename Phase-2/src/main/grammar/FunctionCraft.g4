...
functionDeclaration returns [FunctionDeclaration functionDeclarationRet]:
    ...
    END
    {
        ...
        $functionDeclarationRet.setFunctionName($id.text);
...
patternMatching returns [PatternDeclaration patternRet]:
    ...
    {
        $patternRet = new PatternDeclaration($patternName.text, targetVar);
    }
    RPAR
    (
...
forStatement returns [ForStatement forStRet]:
    ...
    END
    {
        $forStRet = new ForStatement($id.text, $r.rangeRet, $l.loopExps, $l.loopStmts, $l.loopRetStmt);
...
range returns [ArrayList<Expression> rangeRet]:
    ...
    )*) RBRACK)
    {
        ...
        $rangeRet.setLina(e3.getLine());
    }
    |
     id = IDENTIFIER
    {
        $rangeRet.add($id.text);

        $rangeRet.setLina(id.getLine());
    }
    ;
...
matchPatternStatement returns [MatchPatternStatement matchPatRet]:
    ...
    {
        $matchPatRet = new MatchPatternStatement();
        $matchPatRet.setLine(id.getLine($id.text, $e.expRet));
    }
    ;
